...
export default function UnitSelectorToolbar({
    ...
    return (
        <div className="bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 select-none">
            
            {/* 1. Left: Selected Unit Info */}
            <div className="flex items-center gap-3.5 min-w-0 flex-1">
                <div className="w-11 h-11 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center font-bold text-lg shrink-0 shadow-xs">
            ...
            {/* 2. Right: Controls Toolbar (Aligned to the Right with Clean Styling) */}
            <div className="flex items-center gap-3 w-full lg:w-auto shrink-0 justify-end flex-wrap sm:flex-nowrap">
                ...
                <div className="relative flex items-center bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl px-3 py-2 text-xs transition-colors shadow-2xs">
                    <Compass className="w-4 h-4 text-emerald-600 shrink-0 mr-1.5" />
                    <span className="font-bold text-slate-500 mr-1">Radius:</span>
                    <select
                        value={radiusKm}
                        onChange={handleRadiusChange}
                        className="bg-transparent font-extrabold text-slate-900 focus:outline-none cursor-pointer pr-4 appearance-none outline-none border-none py-0"
                    >
                ...
                <div className="relative flex-1 sm:flex-initial min-w-[200px] sm:min-w-[240px]">
                    <select
                        ...
                        className="w-full pl-3.5 pr-8 py-2 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none cursor-pointer transition-all appearance-none shadow-2xs truncate"
                    >
