<x-app-layout title="Dompet Saya">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-gradient-to-br from-primary via-primary-container to-forest-emerald rounded-2xl p-8 text-white shadow-lg overflow-hidden relative">
            ...
            <div class="relative z-10">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('nasabah.topup.form') }}" class="inline-flex items-center gap-2 bg-white text-primary hover:bg-white/90 hover:scale-105 transition-all duration-300 text-sm font-bold px-6 py-3 rounded-full shadow-md">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <!-- Transaction & Top Up History Card -->
                <x-card x-data="{ activeTab: '{{ request()->has('topups_page') ? 'topup' : 'setoran' }}' }">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-4 border-b border-outline-variant">
                        <div>
                            <h2 class="text-lg font-bold text-on-surface">📊 Riwayat Keuangan</h2>
                            <p class="text-xs text-on-surface-variant">Mutasi saldo masuk dan pengisian dompet</p>
                        </div>
                        
                        <!-- Tabs -->
                        <div class="flex bg-surface-container rounded-lg p-1 text-xs font-semibold">
                            <button @click="activeTab = 'setoran'" :class="activeTab === 'setoran' ? 'bg-surface-container-lowest text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'" class="px-3 py-1.5 rounded-md transition-all">♻️ Setoran</button>
                            <button @click="activeTab = 'topup'" :class="activeTab === 'topup' ? 'bg-surface-container-lowest text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'" class="px-3 py-1.5 rounded-md transition-all">💳 Top Up</button>
                        </div>
                    </div>

                    <!-- Setoran Tab -->
                    <div x-show="activeTab === 'setoran'" class="space-y-2">
                        ...
                        @if($mutasi->hasPages())
                            <div class="mt-4 pt-4 border-t border-outline-variant">
                                {{ $mutasi->appends(request()->except('mutasi_page'))->links() }}
                            </div>
                        @endif
                    </div>

                    <!-- Top Up Tab -->
                    <div x-show="activeTab === 'topup'" class="space-y-2" style="display: none;">
                        <div class="space-y-2 max-h-96 overflow-y-auto">
                            @forelse($topups as $topup)
                                <div class="flex items-center justify-between p-4 rounded-lg hover:bg-surface-container-low/50 transition-colors group cursor-pointer">
                                    <div class="flex items-center gap-3 flex-1">
                                        <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <p class="text-sm font-semibold text-on-surface">Top Up Saldo</p>
                                                <x-badge 
                                                    :status="$topup->status === 'pending' ? 'pending' : ($topup->status === 'success' ? 'completed' : 'error')"
                                                    :label="ucfirst($topup->status)"
                                                />
                                            </div>
                                            <p class="text-xs text-on-surface-variant">
                                                {{ $topup->payment_method ? strtoupper(str_replace('_', ' ', $topup->payment_method)) : 'Belum Bayar' }} • {{ $topup->created_at->format('d M Y H:i') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-blue-600">+Rp {{ number_format($topup->nominal, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                            @empty
                                <div class="text-center py-12">
                                    ...
                                    <p class="text-on-surface-variant font-medium">Belum ada top-up</p>
                                    <p class="text-xs text-on-surface-variant">Silakan klik "Top Up Saldo" di atas untuk melakukan pengisian.</p>
                                </div>
                            @endforelse
                        </div>
            ...
            <div class="space-y-6">
                ...
                <x-card id="withdrawal-form">
                    ...
                    <form action="{{ route('nasabah.withdrawal.request') }}" method="POST" class="space-y-4" x-data="{ method: '{{ old('metode', '') }}' }">
                        ...
                        <x-button type="submit" variant="primary" class="w-full" onclick="confirmNasabahWithdrawal(event, this.form)">
                            Ajukan Penarikan
                        </x-button>
                    </form>
                </x-card>
    ...
    @push('scripts')
    <script>
        function confirmNasabahWithdrawal(e, form) {
            e.preventDefault();
            const nominal = form.querySelector('[name="nominal"]').value;
            const metode = form.querySelector('[name="metode"]').value;
            ...
            Swal.fire({
                title: 'Konfirmasi Pengajuan Penarikan',
                html: `Apakah Anda yakin ingin mengajukan pencairan saldo sebesar <b>Rp ${parseInt(nominal).toLocaleString('id-ID')}</b> via <b>${metode.toUpperCase()}</b>?`,
                icon: 'question',
